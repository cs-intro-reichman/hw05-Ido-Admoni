...
playTest(fileName);
	...
	private static void test2(String fileName) {
		...
		System.out.println("cell value: " + cellValue(board,2,2));
}
	...
	public static int[][] read(String fileName) {
		...
		int rows = Integer.parseInt(in.readLine()); //first line of the file -> how many rows
		int cols = Integer.parseInt(in.readLine()); // second line of the file -> how many columns
	...
	public static int cellValue(int[][] board, int i, int j) {
		...
		if (board[i][j] == 1){ // cell is alive (beginning)
			if (aliveNeighbors < 2 || aliveNeighbors >3){
				return 0; // cell dies
			} else {
				return 1; //cell remins alive
			}
		} else { // cell is dead (beginning)
			if (aliveNeighbors == 3){
				return 1; // become alive
			} else {
				return 0; // cell remins dead
			}
		}	
	}
	...
	public static void show1(int[][] board) { //my testing 
		StdDraw.setCanvasSize(900, 900);
		int rows = board.length;
		int cols = board[0].length;
		StdDraw.setXscale(0, cols);
	...
	public static void playTest(String fileName) { // play test with show1
		int[][] board = read(fileName);
		while (true) {
			show1(board);
			board = evolve(board);
